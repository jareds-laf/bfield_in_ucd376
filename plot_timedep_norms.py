...
files = glob.glob(r'G:\My Drive\Other\REUs\Summer 2024\UCD\Data\007\norms_Tek000_*_ALL.csv')
data = []

for i in files:
    if i:
        print(i)
    data.append(pd.read_csv(i,skiprows=14,names=['TIME', 'CH1', 'CH2', 'CH3', 'CH4', 'Norm (V)', 'Norm (B)']))
...
# ax.bar(['Motor', 'Background', 'Paint A', 'Paint B', 'Paint C', 'No Paint'], avgs, color=bar_colors[0:5])
# ax.axhline(y=np.mean(data[1]['Norm (B)']), color='red', linestyle='--', label='Background')
# plt.xticks(rotation=45)

ax.plot(data[0]['TIME'], data[0]['Norm (B)'] / 1e-6, color=bar_colors[0], alpha=0.5, linestyle='-', label='Motor')
